    ...
    const handleSubmitReview = async () => {
        ...
        if (response.ok) {
            ...
            // Delay page reload to show notification
            setTimeout(() => window.location.reload(), 3000);
        } else {
            const error = await response.text();
            showNotification('Failed to submit review: ' + error, 'error');
        }
        } catch (error) {
        console.error(error);
            ...
            {sortedBookings.map((booking) => (
                ...
                <div className={styles.cardDetails}>
                    ...
                    <div className={styles.detailItem}>
                        ...
                        <Check size={16} className={styles.iconGreen} /> {/* Checkmark for rated */}
                    </div>
                    ) : (
